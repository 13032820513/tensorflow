...
# 会话保存地址
model_path = "D:\sample\model5_12.ckpt"
# 输入 matlab 训练数据文件
train_data = sio.loadmat('D:\\MATLAB Workspace\\graduation\\data\\trainData_100000_maxmin.mat')
# 输入 matlab 检验数据文件
check_data = sio.loadmat('D:\\MATLAB Workspace\\graduation\\data\\checkdata3000_01_09package.mat')
...
W6 = tf.Variable(tf.truncated_normal([LAYER5_NODE, OUTPUT_NODE], stddev=0.1))
b6 = tf.Variable(tf.zeros([OUTPUT_NODE])+0.1)
y = tf.nn.relu(tf.matmul(L5_drop, W6)+b6)
