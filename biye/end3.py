...
BATCH_SIZE = 700  # 训练数据批次大小700，400（94%），500（94%）
...
# 会话保存地址
model_path = "D:\sample\model5_14.ckpt"
# 输入 matlab 训练数据文件
train_data = sio.loadmat('D:\\MATLAB Workspace\\graduation\\data\\newTrainData_100000_1_maxmin.mat')
# 输入 matlab 检验数据文件
check_data = sio.loadmat('D:\\MATLAB Workspace\\graduation\\data\\newCheckData_20000_01_09.mat')

#分别获取训练数据的输入和输出，作为神经网络的参数
inputMat = train_data['inputMat01']
outputMat = train_data['outputMat01']

check_input_all= check_data['inputMat']
check_output_all = check_data['outputMat']
check_input = check_input_all[0:2000, :]
check_output = check_output_all[0:2000, OUTPUT_START:OUTPUT_END]
