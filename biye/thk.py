...
LAYER1_NODE = 120  # 第一隐藏层节点数
LAYER2_NODE = 130    # 第二隐藏层节点数
LAYER3_NODE = 700
LAYER4_NODE = 800
LAYER5_NODE = 200
LAYER6_NODE = 100
...
TRAINING_STEPS = 5   # 训练轮数
BATCH_SIZE = 100    # pow(2, 7)  # 训练数据批次大小700，400（94%），500（94%）
...
W8 = tf.Variable(tf.truncated_normal([LAYER7_NODE, LAYER8_NODE], stddev=0.1))
b8 = tf.Variable(tf.zeros([LAYER8_NODE])+0.1)
L8 = tf.nn.tanh(tf.matmul(L7_drop, W8)+b8)
L8_drop = tf.nn.dropout(L8, keep_prob)

W9 = tf.Variable(tf.truncated_normal([LAYER8_NODE, LAYER9_NODE], stddev=0.1))
b9 = tf.Variable(tf.zeros([LAYER9_NODE])+0.1)
L9 = tf.nn.tanh(tf.matmul(L8_drop, W9)+b9)
L9_drop = tf.nn.dropout(L9, keep_prob)


Wn = tf.Variable(tf.truncated_normal([LAYER9_NODE, OUTPUT_NODE], stddev=0.1))
bn = tf.Variable(tf.zeros([OUTPUT_NODE])+0.1)
y = tf.nn.tanh(tf.matmul(L9_drop, Wn)+bn)
    ...
    if isTrain:
        ...
        for i in range(TRAINING_STEPS+1):
            ...
            if i % 1 == 0:
                ...
                plt.title('准确率随训练轮数的变化--test')
                print("验证集：训练 %d 轮后的准确率为=%f" % (i, meanNum))

                # 计算神经网络在训练集上的平均准确率  验证网络是否出现过拟合
                data_train_x = inputMat[0:200,:]
                data_train_y = outputMat[0:200, OUTPUT_START:OUTPUT_END]
                prediction_train = sess.run(y, feed_dict={x: data_train_x})
                accuracy_mean_train = tf.reduce_mean(1 - (tf.abs(prediction_train - data_train_y) / data_train_y))
                meanNum_train = sess.run(accuracy_mean_train)
                plt.scatter(i, meanNum_train, color='red')
                plt.title('准确率随训练轮数的变化——train')
                print("训练集：训练 %d 轮后的准确率为=%f" % (i, meanNum_train))
                print("*****************************************************")
        plt.ylim(0, 1)
        plt.show()
        # ***************************************选取一组数据输出结果***************************************
        check_x = mat(check_input[0, :])
        check_y = mat(check_output[0, :])
        ...
        if flag == 'y':
            ...
            model_path = "D:\sample\model2018520.ckpt"
            save_path = saver.save(sess, model_path)
    else:

        # 使用手动拾取的数据验证模型并且出图
        # 读取实际验证数据
        test_data = sio.loadmat('D:\\MATLAB Workspace\\graduation\\data\\反演thk\\test9.mat')
        test_input = test_data['inputMat']
        test_output = test_data['outputMat']
        # 取出回话的权值
        saver.restore(sess, "D:\sample\model2018520.ckpt")
